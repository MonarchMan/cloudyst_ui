...
export const fileManagerSlice = createSlice({
  ...
  reducers: {
    ...
    setPageSize: (state, action: PayloadAction<FileManagerArgsBase<number>>) => {
      ...
      if (state[index].list?.pagination.next_page_token) {
        // @ts-ignore
        state[index].list.pagination.next_page_token = undefined;
      }
    },
    setSortOption: (state, action: PayloadAction<FileManagerArgsBase<[string, OrderDirection]>>) => {
      const index = action.payload.index;
