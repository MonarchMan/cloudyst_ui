...
import { OrderDirection } from "../../api/dashboard.ts";
  ...
  return async (dispatch, getState) => {
    ...
    if (path && current_fs == Filesystem.share) {
      try {
        ...
        dispatch(setShareReadmeDetect((info?.show_readme ?? false) && info.source_type == FileType.folder));
      } catch (e) {
  ...
  return async (dispatch, getState) => {
    ...
    try {
      listRes = await dispatch(
        getFileList({
          next_page_token: opt && opt.next_page ? list?.pagination.next_page_token : undefined,
          uri: path,
      ...
      if (sortBy == "name" && !getPaginationState(list?.pagination).moreItems) {
        listRes.files = sortByLocalCompare(listRes.files, listRes.mixed_type, orderDirection == OrderDirection.DESC);
      }
    } catch (e) {
    ...
    if (listRes) {
      ...
      if (listRes.view) {
        ...
        dispatch(
          setSortOption({ index, value: [listRes.view.order ?? "created_at", listRes.view.order_direction ?? OrderDirection.ASC] }),
        );
        if (!currentView) {
...
export function loadMorePages(index: number): AppThunk {
  return async (dispatch, getState) => {
    ...
    if (fm.list?.pagination.next_page_token) {
...
export function changeSortOption(index: number, sortBy: string, sortDirection: OrderDirection): AppThunk {
  return async (dispatch, _getState) => {
...
export function syncViewSettings(
  ...
  return async (dispatch, getState) => {
    ...
    const currentView: ExplorerView = {
      ...
      order_direction: fm.sortDirection ?? OrderDirection.ASC,
