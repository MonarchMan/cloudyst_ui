...
export function createShareShortcut(index: number): AppThunk {
  return async (dispatch, getState) => {
    ...
    if (!shareInfo.name || !shareInfo.owner_info) {
      return;
    }
    ...
    await dispatch(
      sendCreateFile({
        ...
        metadata: {
          ...
          [Metadata.share_owner]: shareInfo.owner_id,
        },
        ...
        try {
          ...
          while (true) {
            ...
            if (children.pagination.total_items) {
              page = (page ?? 0) + 1;
            } else if (children.pagination.next_page_token) {
              nextToken = children.pagination.next_page_token;
            }
...
export function importFilesToKnowledge(index: number, files: FileResponse[]): AppThunk {
  return async (dispatch, _getState) => {
    dispatch(closeContextMenu({ index, value: undefined }));

    const candidateFiles = files.filter((file) => file.type == FileType.file && !file.metadata?.[Metadata.restore_uri]);
    if (candidateFiles.length === 0) {
      enqueueSnackbar({
        message: i18next.t("application:ai.noImportableFiles"),
        variant: "warning",
        action: DefaultCloseAction,
      });
      return;
    ...
    const knowledgeRes = await dispatch(
      listKnowledge({
        pagination: {
          page_size: 100,
        },
    ...
    if (knowledgeRes.knowledges.length === 0) {
      enqueueSnackbar({
        message: i18next.t("application:ai.noKnowledgeAvailable"),
        variant: "warning",
        action: DefaultCloseAction,
      });
      return;
    ...
    let knowledgeId = "";
    try {
      knowledgeId = await dispatch(
        selectOption(
          knowledgeRes.knowledges.map((knowledge) => ({
            name: knowledge.name,
            description: knowledge.description || knowledge.embedding_model,
            value: knowledge.id,
          })),
          "application:ai.selectKnowledgeBase",
        ),
      );
    } catch (_e) {
      return;
    ...
    try {
      const res = await dispatch(
        createMultiDocuments({
          documents: candidateFiles.map((file) => ({
      ...
      importedCount = res.documents?.length ?? candidateFiles.length;
      failedCount = Math.max(candidateFiles.length - importedCount, 0);
    } catch (_e) {
      importedCount = 0;
    }
    ...
    if (importedCount === 0) {
      enqueueSnackbar({
        message: i18next.t("application:ai.importSubmitFailed"),
        variant: "error",
        action: DefaultCloseAction,
      });
      return;
    ...
    enqueueSnackbar({
      message: i18next.t(
        failedCount > 0 ? "application:ai.importQueuedPartialSuccess" : "application:ai.importQueuedSuccess",
        {
