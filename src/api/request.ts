...
export interface Response<T> {
   ...
   *
   * For batch operations, this is now also used to carry aggregated
   * error details instead of the old `aggregated_error` field.
   */
...
export class AppError extends Error {
  ...
  public metadata: Map<string, string> | undefined = undefined;
  ...
  constructor(resp: Response<any>) {
    ...
    this.metadata = resp.metadata as Map<string, string> | undefined;
...
/**
 * Helper to decode aggregated errors from the new metadata field.
 *
 * The backend now encodes aggregated errors as a map of
 *   path -> JSON-serialized Response<any>
 * in `metadata`. This function parses that structure back into a
 * typed `AggregatedError` object so existing UI code can keep using
 * the old shape.
 */
export function getAggregatedErrorsFromMetadata<T = any>(
  error: Response<any>,
): AggregatedError<T> | undefined {
  if (!error.metadata) {
    return undefined;
  }

  const result: AggregatedError<T> = {};

  // Support both real Map instances and plain objects (what axios usually gives us)
  const entries =
    error.metadata instanceof Map
      ? Array.from(error.metadata.entries())
      : Object.entries(error.metadata as Record<string, string>);

  for (const [key, value] of entries) {
    if (!value) continue;
    try {
      const parsed = JSON.parse(value) as Response<T>;
      // Require at least a code/msg to look like a Response
      if (typeof parsed === "object" && parsed !== null && "code" in parsed && "msg" in parsed) {
        result[key] = parsed;
      }
    } catch {
      // Ignore invalid JSON entries – they are not aggregated error payloads
      continue;
    }
  }
