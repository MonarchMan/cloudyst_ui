...
const ContextMenu = ({ fmIndex = 0 }: ContextMenuProps) => {
  ...
  let part2 =
    ...
    displayOpt.showImportToKnowledge ||
  ...
  const part2Elements = part2 ? (
    <>
      ...
      {displayOpt.showImportToKnowledge && (
        <SquareMenuItem onClick={() => dispatch(importFilesToKnowledge(fmIndex, targets))}>
          <ListItemIcon>
            <Book fontSize="small" />
          </ListItemIcon>
          <ListItemText>
            {targets.length > 1 ? t("application:ai.batchImportToKnowledge") : t("application:ai.addToKnowledge")}
          </ListItemText>
        </SquareMenuItem>
      )}
      {displayOpt.showRename && (
