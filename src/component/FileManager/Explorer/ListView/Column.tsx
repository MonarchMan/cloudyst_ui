...
export interface ColumnProps {
  ...
  setSortBy?: (order_by: string, order_direction: OrderDirection) => void;
}
...
const SortArrow = styled(ArrowSortDownFilled)<{
  ...
  transform: `rotate(${direction === OrderDirection.ASC ? 180 : 0}deg)`,
...
const Column = ({ column, showDivider, index, startResizing, sortDirection, setSortBy, sortable }: ColumnProps) => {
  ...
  const onSortOptionChange = useCallback(() => {
    ...
    const newDirection = sortDirection === OrderDirection.ASC ? OrderDirection.DESC : OrderDirection.ASC;
