...
import { Group } from "../../../../api/dashboard";
...
const FileManagementSection = () => {
  ...
  const onCompressSizeChange = useCallback(
    (e: number) => {
      setGroup((p: Group) => ({ ...p, settings: { ...p.settings, compress_size: e ? e : undefined } }));
    },
    [setGroup],
  );
  ...
  const onDecompressSizeChange = useCallback(
    (e: number) => {
      setGroup((p: Group) => ({ ...p, settings: { ...p.settings, decompress_size: e ? e : undefined } }));
    },
    [setGroup],
  );
  ...
  const onEditedConfigBlur = useCallback(
    (value: string) => {
      ...
      setGroup((p: Group) => ({ ...p, settings: { ...p.settings, remote_download_options: res } }));
    },
