...
const FileSetting = () => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = files.map((n) => n.file.id);
        ...
        <TableContainer component={StyledTableContainerPaper} sx={{ mt: 2 }}>
          <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
            ...
            <TableBody>
              {!loading &&
                files.map((file) => (
                  <FileRow
                    ...
                    file={file}
                    onDelete={fetchFiles}
                    selected={selected.includes(file.file.id)}
              ...
              {loading &&
                files.length > 0 &&
                files.slice(0, 10).map((file) => <FileRow key={`loading-${file.file.id}`} loading={true} />)}
              {loading &&
                files.length === 0 &&
