...
const FileForm = () => {
  ...
  const userClicked = (e: React.MouseEvent<HTMLElement>) => {
    ...
    setUserDialogID(values?.file?.owner_id ?? 0);
  };

  const sizeUsed = useMemo(() => {
    return sizeToString(values?.file.entities?.reduce((acc, entity) => acc + (entity.size ?? 0), 0) ?? 0);
  }, [values?.file.entities]);
      ...
      <Box component={"form"} ref={formRef} onSubmit={(e) => e.preventDefault()}>
        <Grid container spacing={isMobile ? 2 : 3} alignItems={"stretch"}>
          ...
          <SettingForm title={t("file.size")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {sizeToString(values.file.size ?? 0)}
            </Typography>
          </SettingForm>
          ...
          <SettingForm title={t("file.shareLink")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              <Trans
                ...
                components={[
                  <Link component={RouterLink} to={`/admin/share?file=${values?.file.id}`} underline={"hover"} key={0} />,
                ]}
                ns="dashboard"
                values={{ num: values.file?.shares?.length ?? 0 }}
              />
            </Typography>
          </SettingForm>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <UserAvatar
                  ...
                  user={{
                    ...
                    nickname: values?.file?.owner_info?.nickname ?? "",
                    created_at: values?.file?.owner_info?.created_at ?? "",
                  }}
                />
                <NoWrapTypography variant="inherit">
                  <Link
                    ...
                    href="#/"
                  >
                    {values?.file?.owner_info?.nickname ?? ""}
                  </Link>
                </NoWrapTypography>
              </Box>
            </NoWrapTypography>
          ...
          <SettingForm title={t("file.name")} noContainer lgWidth={6}>
            <DenseFilledTextField fullWidth value={values.file.name} required onChange={onNameChange} />
          </SettingForm>
          ...
          <SettingForm title={t("file.primaryStoragePolicy")} noContainer lgWidth={3} pro>
            <SinglePolicySelectionInput simplified value={values.file.storage_policy_files ?? 0} onChange={() => {}} />
          </SettingForm>
