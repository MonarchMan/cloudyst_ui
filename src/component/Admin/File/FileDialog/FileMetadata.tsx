...
const FileMetadata = () => {
  ...
  const onKeyChange = (index: number, value: string) => {
    setFile((prev) => ({
      ...prev,
      metadata: prev.file?.metadata?.map((item, i) => (i === index ? { ...item, name: value } : item)),
    }));
  };
  const onValueChange = (index: number, value: string) => {
    setFile((prev) => ({
      ...prev,
      metadata: prev.file?.metadata?.map((item, i) => (i === index ? { ...item, value: value } : item)),
    }));
  };
  const onIsPublicChange = (index: number, value: boolean) => {
    setFile((prev) => ({
      ...prev,
      metadata: prev.file?.metadata?.map((item, i) =>
  ...
  const handleDelete = (id: number) => {
    setFile((prev) => ({
      ...prev,
      metadata: prev.file.metadata?.filter((item) => item.id !== id)
    }));
  };
  return (
    <TableContainer component={StyledTableContainerPaper} sx={{ maxHeight: "300px" }}>
      <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
        ...
        <TableBody>
          {values?.file?.metadata?.map((option, index) => (
