...
const UserForm = ({ reload, setLoading }: { reload: () => void; setLoading: (loading: boolean) => void }) => {
  ...
  const onCalibrateStorage = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>) => {
      e.preventDefault();
      setLoading(true);
      dispatch(sendCalibrateUserStorage(values.user.id))
        .then(() => {
          reload();
          enqueueSnackbar({
        ...
        <Stack spacing={isMobile ? 2 : 3} direction={"column"} sx={{ minWidth: 200 }}>
          <SettingForm title={t("user.avatar")} noContainer lgWidth={12}>
            <UserAvatar
              ...
              user={{
                ...
                nickname: values.user.nick ?? "",
                id: values.user.avatar ? values.hash_id ?? "" : "dummy",
              }}
            />
            <Collapse in={!!values.user.avatar} unmountOnExit>
          ...
          <SettingForm title={t("user.id")} noContainer lgWidth={12}>
            <Typography variant="body2" color="text.secondary">
              {t("user.idValue", { id: values.user.id, hash_id: values.hash_id })}
            </Typography>
          </SettingForm>
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Grid container spacing={isMobile ? 2 : 3}>
            <SettingForm title={t("user.email")} noContainer lgWidth={6}>
              <DenseFilledTextField fullWidth type="email" value={values.user.email} required onChange={onEmailChange} />
            </SettingForm>
            <SettingForm title={t("user.nick")} noContainer lgWidth={6}>
              <DenseFilledTextField fullWidth value={values.user.nick} required onChange={onNickChange} />
            </SettingForm>
            <SettingForm title={t("user.userStatus")} noContainer lgWidth={6}>
              <FormControl fullWidth>
                <DenseSelect value={values.user.status} onChange={onStatusChange}>
            ...
            <SettingForm title={t("user.group")} noContainer lgWidth={6}>
              <GroupSelectionInput value={values.user.group_users?.toString() ?? ""} onChange={onGroupChange} fullWidth />
            </SettingForm>
