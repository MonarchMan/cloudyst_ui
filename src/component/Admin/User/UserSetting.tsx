...
import { OrderDirection, User } from "../../../api/dashboard";
...
const UserSetting = () => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = users.map((n) => n.user.id);
        ...
        <TableContainer component={StyledTableContainerPaper} sx={{ mt: 2 }}>
          <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
            ...
            <TableBody>
              {!loading &&
                users.map((user) => (
                  <UserRow
                    ...
                    user={user}
                    onDelete={fetchUsers}
                    selected={selected.includes(user.user.id)}
              ...
              {loading &&
                users.length > 0 &&
                users.slice(0, 10).map((user) => <UserRow key={`loading-${user.user.id}`} loading={true} />)}
              {loading &&
                users.length === 0 &&
