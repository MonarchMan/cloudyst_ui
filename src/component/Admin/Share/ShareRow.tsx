...
const ShareRow = ({
  ...
  const onDeleteClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    dispatch(confirmOperation(t("share.confirmDelete"))).then(() => {
      if (share?.share.id) {
        ...
        dispatch(batchDeleteShares({ ids: [share.share.id] }))
          .then(() => {
            onDelete?.();
          })
          .finally(() => {
  ...
  const userClicked = (e: React.MouseEvent<HTMLElement>) => {
    e.stopPropagation();
    e.preventDefault();
    openUserDialog?.(share?.share.owner_id ?? 0);
  };

  const fileClicked = (e: React.MouseEvent<HTMLElement>) => {
    e.stopPropagation();
    e.preventDefault();
    openFileDialog?.(share?.share.file?.id ?? 0);
  };
  ...
  return (
    <TableRow hover key={share?.share.id} sx={{ cursor: "pointer" }} onClick={onRowClick} selected={selected}>
      ...
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{share?.share.id}</NoWrapTypography>
      </NoWrapTableCell>
      <TableCell>
        {share?.share.file ? (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <FileTypeIcon name={share?.share.file?.name ?? ""} fileType={share?.share.file?.type ?? 0} />
      ...
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{share?.share.views ?? 0}</NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{share?.share.downloads ?? 0}</NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{share?.share.price}</NoWrapTypography>
      </NoWrapTableCell>
      <TableCell>
        <ShareExpires expires={share?.share.expires} remain_downloads={share?.share.remain_downloads} />
      </TableCell>
      <NoWrapTableCell>
        {share?.share.owner_info && (
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <UserAvatar
              ...
              user={{
                ...
                nickname: share?.share.owner_info?.nickname ?? "",
                created_at: "",
              }}
            />
            <NoWrapTypography variant="inherit">
              <Link
                ...
                href="#/"
              >
                {share?.share.owner_info?.nickname}
              </Link>
            </NoWrapTypography>
          </Box>
        )}
      ...
      <NoWrapTableCell>
        <TimeBadge datetime={share?.share.created_at ?? ""} variant="inherit" timeAgoThreshold={0} />
      </NoWrapTableCell>
      <NoWrapTableCell>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
