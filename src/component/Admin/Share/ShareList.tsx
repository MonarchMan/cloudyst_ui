...
import { AdminListService, OrderDirection, Share } from "../../../api/dashboard";
...
const ShareList = () => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = shares.map((n) => n.share.id);
        ...
        <TableContainer component={StyledTableContainerPaper} sx={{ mt: 2 }}>
          <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
            ...
            <TableBody>
              {!loading &&
                shares.map((share) => (
                  <ShareRow
                    ...
                    selected={selected.indexOf(share.share.id) !== -1}
              ...
              {loading &&
                shares.length > 0 &&
                shares.slice(0, 10).map((share) => <ShareRow key={`loading-${share.share.id}`} loading={true} />)}
              {loading &&
                shares.length === 0 &&
