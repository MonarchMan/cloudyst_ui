...
import { AdminListService, Entity, OrderDirection } from "../../../api/dashboard";
...
const EntitySetting = () => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = entities.map((n) => n.entity.id);
      ...
      <Container maxWidth="xl">
        ...
        <TableContainer component={StyledTableContainerPaper} sx={{ mt: 2 }}>
          <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
            ...
            <TableBody>
              {!loading &&
                entities.map((entityEnt) => (
                  <EntityRow
                    key={entityEnt.entity.id}
                    entity={entityEnt}
                    onDelete={handleSingleDelete}
                    selected={selected.indexOf(entityEnt.entity.id) !== -1}
              ...
              {loading &&
                entities.length > 0 &&
                entities.slice(0, 10).map((entity) => <EntityRow key={`loading-${entity.entity.id}`} loading={true} />)}
              {loading &&
                entities.length === 0 &&
