...
const EntityRow = ({
  ...
  const onOpenClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    dispatch(getEntityUrl(entity?.entity.id ?? 0))
      .then((res) => {
        // 直接下载文件：使用a标签的download属性强制下载
        const link = document.createElement('a');
        link.href = res.url;
        link.download = `entity-${entity?.entity.id}`;
  ...
  const userClicked = (e: React.MouseEvent<HTMLElement>) => {
    e.stopPropagation();
    e.preventDefault();
    openUserDialog?.(entity?.entity.created_by ?? 0);
  };

  const onDeleteClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    e.preventDefault();
    onDelete?.(entity?.entity.id ?? 0);
  };
  ...
  return (
    <TableRow
      ...
      sx={{ cursor: "pointer" }}
      onClick={() => openEntityDialog?.(entity?.entity.id ?? 0)}
      selected={selected}
    >
      ...
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{entity?.entity.id}</NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{t(EntityTypeText[entity?.entity.type ?? EntityType.version])}</NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Tooltip title={entity?.entity.source || ""}>
            <NoWrapTypography variant="inherit">{entity?.entity.source || "-"}</NoWrapTypography>
          </Tooltip>
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
            {!entity?.entity.reference_count && <SquareChip size="small" label={t("entity.waitForRecycle")} />}
          </Box>
        </Box>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{sizeToString(entity?.entity.size ?? 0)}</NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">
          <Link
            ...
            underline="hover"
            to={`/admin/policy/${entity?.entity.storage_policy?.id}`}
          >
            {entity?.entity.storage_policy?.name || "-"}
          </Link>
        </NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">{entity?.entity.reference_count ?? 0}</NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <NoWrapTypography variant="inherit">
          <TimeBadge datetime={entity?.entity.created_at ?? ""} variant="inherit" timeAgoThreshold={0} />
        </NoWrapTypography>
      </NoWrapTableCell>
      <NoWrapTableCell>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
