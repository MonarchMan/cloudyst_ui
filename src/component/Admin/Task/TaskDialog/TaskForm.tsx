...
const TaskForm = ({ values }: { values: Task }) => {
  ...
  const userClicked = (e: React.MouseEvent<HTMLElement>) => {
    ...
    setUserDialogID(values?.task.user_id ?? 0);
  };
  ...
  const privateState = useMemo((): any => {
    let res: any = {};
    if (values.task.private_state) {
      try {
        res = JSON.parse(values.task.private_state);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  const isUserTask = useMemo(() => {
    return userTaskTypes.includes(values.task.type ?? "");
  }, [values]);
      ...
      <Box>
        <Grid container spacing={isMobile ? 2 : 3} alignItems={"stretch"}>
          ...
          <SettingForm title={t("task.type")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {t(`task.${values.task.type}`)}
            </Typography>
          </SettingForm>

          <SettingForm title={t("task.status")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {getTaskStatusText(values?.task.status ?? TaskStatus.queued, t)}
            </Typography>
          </SettingForm>
          ...
          <SettingForm title={t("task.user_id")} noContainer lgWidth={2}>
            <Typography variant="body2" color={"textSecondary"}>
              {values.task.user_id}
            </Typography>
          </SettingForm>
          ...
          <SettingForm title={t("user.createdAt")} noContainer lgWidth={4}>
            <Typography variant={"body2"} color={"textSecondary"}>
              <TimeBadge datetime={values?.task.created_at ?? ""} variant="inherit" timeAgoThreshold={0} />
            </Typography>
          </SettingForm>

          <SettingForm title={t("task.updatedAt")} noContainer lgWidth={4}>
            <Typography variant={"body2"} color={"textSecondary"}>
              <TimeBadge datetime={values?.task.updated_at ?? ""} variant="inherit" timeAgoThreshold={0} />
            </Typography>
          </SettingForm>

          <SettingForm title={t("event.correlationId")} noContainer lgWidth={4}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {values?.task.correlation_id ?? "-"}
            </Typography>
          </SettingForm>

          <SettingForm title={t("application:setting.executeDuration")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {formatDuration(dayjs.duration((values?.task.public_state?.executed_duration ?? 0) / 1e6))}
            </Typography>
          </SettingForm>

          <SettingForm title={t("application:setting.retryCount")} noContainer lgWidth={2}>
            <Typography variant={"body2"} color={"textSecondary"}>
              {values?.task.public_state?.retry_count ?? "-"}
            </Typography>
          </SettingForm>
            ...
            <SettingForm title={t("application:setting.output")} noContainer lgWidth={4}>
              <FileBadge
                ...
                simplifiedFile={{
                  ...
                  type: values?.task.type == TaskType.extract_archive ? FileType.folder : FileType.file,
                }}
              />
            </SettingForm>
          )}

          {values?.task.public_state?.error && (
          ...
          {isUserTask && (
            <SettingForm title={t("application:setting.taskProgress")} noContainer lgWidth={12}>
              <TaskProgress
                ...
                taskStatus={values.task.status ?? TaskStatus.queued}
          ...
          {values?.task.public_state?.error_history && (
            <SettingForm title={t("application:setting.retryErrorHistory")} noContainer lgWidth={12}>
              <TableContainer component={StyledTableContainerPaper}>
                <Table sx={{ width: "100%" }} size="small">
                  ...
                  <TableBody>
                    {values?.task.public_state?.error_history.map((error, index) => (
          ...
          {values.task.type == TaskType.entity_recycle_routine && privateState?.errors && (
