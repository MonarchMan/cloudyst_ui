...
import TaskServiceSelector from "./TaskServiceSelector";
...
const TaskList = () => {
  ...
  const [service, setService] = useQueryState(ServiceQuery, { defaultValue: ServiceName.file });
  ...
  const currentService = Object.values(ServiceName).includes(service as ServiceName) ? (service as ServiceName) : ServiceName.file;

  useEffect(() => {
    fetchTasks();
  }, [page, pageSize, orderBy, orderDirection, user, type, status, correlationID, currentService]);
  ...
  const fetchTasks = () => {
    ...
    dispatch(getTaskList(params, currentService))
      .then((res) => {
        setTasks(res.tasks);
  ...
  const handleDelete = () => {
    ...
    dispatch(confirmOperation(t("task.confirmBatchDelete", { num: selected.length })))
      .then(() => {
        dispatch(batchDeleteTasks({ ids: Array.from(selected) }, currentService))
          .then(() => {
            fetchTasks();
          })
          .finally(() => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = tasks.map((n) => n.task.id);
  ...
  const handleServiceChange = (nextService: ServiceName) => {
  ...
  return (
    <PageContainer>
      ...
      <TaskDialog open={openTaskDialogOpen} onClose={() => setOpenTaskDialogOpen(false)} taskID={openTask} service={currentService} />
      <TaskCleanupDialog
      ...
      <Container maxWidth="xl">
        ...
        <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
          <TaskServiceSelector value={currentService} onChange={handleServiceChange} />
        ...
        <TableContainer component={StyledTableContainerPaper} sx={{ mt: 2 }}>
          <Table size="small" stickyHeader sx={{ width: "100%", tableLayout: "fixed" }}>
            ...
            <TableBody>
              {!loading &&
                tasks.map((task) => (
                  <TaskRow
                    ...
                    service={currentService}
                    onDelete={fetchTasks}
                    selected={selected.indexOf(task.task.id) !== -1}
              ...
              {loading &&
                tasks.length > 0 &&
                tasks.slice(0, 10).map((task) => <TaskRow key={`loading-${task.task.id}`} loading={true} />)}
              {loading &&
                tasks.length === 0 &&
