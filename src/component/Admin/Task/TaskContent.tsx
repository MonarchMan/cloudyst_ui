...
export const TaskContent = memo(({ task, openEntity }: TaskContentProps) => {
  ...
  const content = useMemo(() => {
    ...
    try {
      privateState = JSON.parse(task.task.private_state ?? "{}");
    } catch (error) {
      console.error(error);
    }
  ...
  if (userTaskTypes.includes(task.task.type ?? "")) {
    return (
      <Typography variant="body2">
        <TaskSummaryTitle type={task.task.type?.toString() ?? ""} summary={processedSummary} isInDashboard />
      </Typography>
    );
  }
