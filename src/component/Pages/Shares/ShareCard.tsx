    ...
    <>
      ...
      <Grid item xs={12} sm={6} md={4} ref={ref}>
        <SummaryButton
          ...
          onContextMenu={(e) => {
            e.preventDefault();
            if (share?.owner_id === user?.user.id) {
              // ensure anchor is the element that received the event and open the menu
              try {
                // use currentTarget to anchor to the element
                popupState.setAnchorEl?.(e.currentTarget as EventTarget as HTMLElement);
              } catch (err) {
                // ignore
              }
              popupState.open?.();
